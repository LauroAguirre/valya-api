...
export const evolutionService = {
  /**
   * Criar instancia do WhatsApp na EvolutionAPI para um corretor
   */
  ...
  /**
   * Obter QR Code atual da instancia
   */
  async getQrCode(userId: string) {
    ...
    const res = await fetch(`${EVOLUTION_URL}/instance/connect/${config.instanceName}`, {
      method: 'GET',
      headers: headers(),
    });
  ...
  async getConnectionStatus(userId: string) {
    ...
    const res = await fetch(`${EVOLUTION_URL}/instance/connectionState/${config.instanceName}`, {
      method: 'GET',
      headers: headers(),
    });

    if (!res.ok) {
      throw new Error('Erro ao verificar status da conexao.');
    }
  ...
  /**
   * Enviar mensagem via EvolutionAPI com simulacao de digitacao
   */
  async sendMessage(instanceName: string, to: string, message: string) {
    const phone = normalizePhone(to);

    // Simular "digitando" conforme tamanho da mensagem
    ...
    try {
      ...
      // Nao bloquear envio se falhar o typing
    }

    // Aguardar delay proporcional
  ...
  /**
   * Encontrar instancia do corretor pelo telefone destinatario
   */
  async findBrokerByPhone(phone: string) {
    ...
    const config = await prisma.evolutionConfig.findFirst({
      where: {
        phone: { contains: normalizedPhone.slice(-8) }, // Ultimos 8 digitos
        connected: true,
      },
      include: {
        user: {
