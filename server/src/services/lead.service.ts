...
export const leadService = {
  ...
  async create(
    ...
    const lead = await prisma.lead.create({
      data: {
        ...
        phone: data.phone,
        email: data.email,
        origin: (data.origin as any) || 'WHATSAPP',
        stage: data.stage || 'QUALIFICACAO',
      },
    });
  ...
  async update(
    ...
    const updated = await prisma.lead.update({
      ...
      data: {
        ...data,
        origin: data.origin ? (data.origin as any) : undefined,
      },
    });
  ...
  /**
   * Encontra ou cria um lead a partir do telefone de entrada do WhatsApp
   */
