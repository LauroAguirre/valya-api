...
export const propertyService = {
  async list(userId: string, params: { page?: number; limit?: number; search?: string }) {
    ...
    const [properties, total] = await Promise.all([
      prisma.property.findMany({
        where,
        include: { images: { take: 1, orderBy: { order: 'asc' } }, units: { select: { id: true } } },
  ...
  async getById(userId: string, propertyId: string) {
    const property = await prisma.property.findFirst({
      ...
      include: { units: { orderBy: { unitName: 'asc' } }, images: { orderBy: { order: 'asc' } }, adLinks: true },
    });
  ...
  async create(userId: string, data: CreatePropertyData) {
    return prisma.property.create({ data: { userId, ...data } });
  },

  async update(userId: string, propertyId: string, data: Partial<CreatePropertyData>) {
    const existing = await prisma.property.findFirst({ where: { id: propertyId, userId } });
  ...
  async createUnit(userId: string, propertyId: string, data: PropertyUnitData) {
    ...
    if (!property) throw new Error('Imovel nao encontrado.');
    return prisma.propertyUnit.create({ data: { propertyId, ...data } });
  },

  async updateUnit(userId: string, unitId: string, data: Partial<PropertyUnitData>) {
    const unit = await prisma.propertyUnit.findUnique({ where: { id: unitId }, include: { property: true } });
    if (!unit || unit.property.userId !== userId) throw new Error('Unidade nao encontrada.');
    return prisma.propertyUnit.update({ where: { id: unitId }, data });
  },
  ...
  async bulkCreateUnits(userId: string, propertyId: string, units: PropertyUnitData[]) {
    ...
    const created = await prisma.propertyUnit.createMany({ data: units.map((u) => ({ propertyId, ...u })) });
    return { count: created.count };
  },

  async addImages(userId: string, propertyId: string, images: { url: string; description?: string }[]) {
    const property = await prisma.property.findFirst({ where: { id: propertyId, userId } });
    if (!property) throw new Error('Imovel nao encontrado.');
    const maxOrder = await prisma.propertyImage.findFirst({ where: { propertyId }, orderBy: { order: 'desc' }, select: { order: true } });
    let nextOrder = (maxOrder?.order ?? -1) + 1;
    const created = await prisma.propertyImage.createMany({
      data: images.map((img) => ({ propertyId, url: img.url, description: img.description, order: nextOrder++ })),
    });
    return { count: created.count };
  },
  ...
  async updateImage(userId: string, imageId: string, data: { description?: string; order?: number }) {
    ...
    return prisma.propertyImage.update({ where: { id: imageId }, data });
  },
  ...
  async setAdLinks(userId: string, propertyId: string, adLinks: AdLinkData[]) {
    ...
    const created = await prisma.propertyAdLink.createMany({ data: adLinks.map((link) => ({ propertyId, ...link })) });
    return { count: created.count };
  },
};
