...
export const authService = {
  ...
  async oauthLogin(provider: 'GOOGLE' | 'FACEBOOK', accessToken: string) {
    ...
    if (provider === 'GOOGLE') {
      ...
      profileData = { email: data.email, name: data.name, providerId: data.id, avatarUrl: data.picture };
    } else {
      ...
      const data = await res.json();
      profileData = { email: data.email, name: data.name, providerId: data.id, avatarUrl: data.picture?.data?.url };
    }

    if (!profileData.email) throw new Error('Nao foi possivel obter o e-mail do provedor OAuth.');
  ...
  async getMe(userId: string) {
    const user = await prisma.user.findUnique({
      ...
      select: {
        ...
        personalPhone: true, address: true, avatarUrl: true, createdAt: true,
