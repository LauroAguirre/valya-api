...
export const profileService = {
  async getProfile(userId: string) {
    const user = await prisma.user.findUnique({
      ...
      select: {
        ...
        address: true, avatarUrl: true, role: true, createdAt: true,
  ...
  async updateProfile(userId: string, data: { name?: string; phone?: string; personalPhone?: string; address?: string }) {
    return prisma.user.update({
      where: { id: userId },
      data,
      select: { id: true, name: true, email: true, phone: true, personalPhone: true, address: true, avatarUrl: true },
    });
  },
  ...
  async getSubscriptionStatus(userId: string) {
    ...
    return {
      ...
      needsRenewalWarning: daysUntilExpiry <= 10 && daysUntilExpiry > 0 && !asaasCustomer,
    };
  },
};
