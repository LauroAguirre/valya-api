...
export const asaasService = {
  async createCustomer(userId: string, data: {
    ...
    postalCode?: string; address?: string; addressNumber?: string;
  }) {
    ...
    const res = await fetch(`${ASAAS_URL}/customers`, {
      ...
      body: JSON.stringify({ name: data.name, cpfCnpj: data.cpfCnpj, email: data.email, phone: data.phone, postalCode: data.postalCode, address: data.address, addressNumber: data.addressNumber, externalReference: userId }),
    });
    ...
    return prisma.asaasCustomer.create({
      data: { userId, asaasCustomerId: customer.id, cpfCnpj: data.cpfCnpj, name: data.name, email: data.email, phone: data.phone, postalCode: data.postalCode, address: data.address, addressNumber: data.addressNumber },
    });
  },

  async createSubscription(userId: string, params: { value: number; dueDate: string; description?: string }) {
    ...
    const res = await fetch(`${ASAAS_URL}/subscriptions`, {
      ...
      body: JSON.stringify({
        ...
        description: params.description || 'Assinatura Valya - Plano Mensal', externalReference: userId,
      }),
    });
    ...
    await prisma.subscription.update({ where: { userId }, data: { asaasSubId: subscription.id, status: 'ACTIVE' } });
    return subscription;
  },
  ...
  async handlePaymentWebhook(event: string, payment: Record<string, unknown>) {
    ...
    const user = await prisma.user.findUnique({ where: { id: externalReference }, include: { subscription: true } });
